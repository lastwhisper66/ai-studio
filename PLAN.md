...
AI Studio 是一个 Windows 桌面 AI 聊天应用，基于 Electron + React 19。项目目录当前只有 CLAUDE.md 架构文档，没有任何源代码。本计划将项目分为 **7 个递增交付阶段**，每个阶段结束后应用都可运行。
...
1. **初始化项目** - 使用 `pnpm create @quick-start/electron` 脚手架（react-ts 模板），将生成的文件合并到当前目录（保留已有 CLAUDE.md 和 .claude/）
2. **调整 `package.json`** - name 设为 `ai-studio`，确认 scripts（dev/build/typecheck/lint）
3. **配置 TypeScript** - 三个 tsconfig（根、node、web），全部 `strict: true`，web 用 `moduleResolution: "bundler"`
4. **配置 `electron.vite.config.ts`** - main/preload 用 `externalizeDepsPlugin()`，renderer 用 `@vitejs/plugin-react` + 路径别名
5. **编写最小入口**：
   - `src/main/index.ts` - 创建 BrowserWindow，加载 renderer
   - `src/preload/index.ts` - 空 contextBridge 占位
   - `src/renderer/src/App.tsx` - 显示 "AI Studio" 标题
6. **配置 `electron-builder.yml`** - Windows NSIS 打包基础配置
7. **配置 ESLint** + `.gitignore`

### 依赖

```
核心：electron, electron-vite, @vitejs/plugin-react, typescript, react, react-dom
开发：electron-builder, @electron-toolkit/preload, @electron-toolkit/utils, eslint
...
1. **安装 Tailwind CSS v4** - `tailwindcss` + `@tailwindcss/vite`，插件仅加到 renderer 配置
2. **创建 `src/renderer/src/assets/main.css`** - `@import "tailwindcss"` + `@theme` 定义设计令牌 + `.dark` 变量
3. **初始化 Shadcn/UI** - 创建临时 `vite.config.ts` 运行 `npx shadcn@latest init`，配置 `components.json` 的 aliases，完成后删除临时文件
4. **安装 Shadcn 组件** - button, input, scroll-area, separator, tooltip, dialog, dropdown-menu, textarea, avatar
5. **创建布局组件**：
   - `src/renderer/src/components/layout/AppLayout.tsx` - flex 两栏布局
   - `src/renderer/src/components/layout/Sidebar.tsx` - 左侧 280px 面板（静态占位）
   - `src/renderer/src/components/layout/ChatPanel.tsx` - 右侧 flex-1（静态占位）
6. **实现主题切换** - `ThemeProvider.tsx` + `useTheme.ts`，class 策略切换 `<html>` 的 `dark` class
7. **创建 `src/renderer/src/lib/utils.ts`** - `cn()` 工具函数
...
| 阶段       | 核心交付物                   | 关键难点                            |
| ---------- | ---------------------------- | ----------------------------------- |
| 1. 脚手架  | 空白 Electron 窗口           | electron-vite 配置                  |
| 2. UI 基础 | Tailwind + Shadcn + 分栏布局 | Tailwind v4 CSS-first + Shadcn 兼容 |
| 3. 数据层  | SQLite + IPC + 对话 CRUD     | better-sqlite3 native rebuild       |
| 4. AI 核心 | 流式对话                     | OpenAI streaming over IPC           |
| 5. 设置    | API 配置页面                 | safeStorage 加密                    |
| 6. 渲染    | Markdown + 代码高亮          | 流式 Markdown 性能                  |
| 7. 发布    | Windows 安装包               | 安全加固 + 打包配置                 |
